...
from compressed_sensing import compressed_sensing_anomaly_score
...
def run_for_window(returns, window_days):
    ...
    for ticker in ret_window.columns:
        s = compressed_sensing_anomaly_score(
            ret_window[ticker],
            measurement_ratio=config.MEASUREMENT_RATIO,
    ...
    top_etfs = [{"ticker": t, "cs_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/compressed_sensing_{timestamp}.json"
